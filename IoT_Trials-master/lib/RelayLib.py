...
libpath = os.path.join(os.path.dirname(__file__), "usb_relay_hid")
...
def timer_relay_trigger(num,interval):
    global hdev
    loadLib()
    getLibFunctions()
    try:
      ...
      if len(devids) != 0 :
        ...
        time.sleep(interval)
        ret = ret + L.usb_relay_device_close_one_relay_channel(hdev,num)
        if ret != 0:
            fail("Failed to trigger timer switch for R"+str(num))
        else:
            print("Triggered timer switch on R"+str(num)+"!")
      closeDev()
    finally:
      unloadLib()

def turn_on_relay(num):
  global hdev
  loadLib()
  getLibFunctions()
  try:
    ...
    if len(devids) != 0 :
      ...
      ret = L.usb_relay_device_open_one_relay_channel(hdev,num)
      if ret != 0:
        fail("Failed to turn on R"+str(num))
      else:
        print("Turned on R"+str(num)+"!")
      closeDev()
  finally:  
    unloadLib()

def turn_off_relay(num):
  global hdev
  loadLib()
  getLibFunctions()
  try:
    ...
    if len(devids) != 0 :
      ...
      if ret != 0:
        fail("Failed to turn off R1"+str(num))
      else:
        print("Turned on R"+str(num)+"!")
      closeDev()
  finally:  
